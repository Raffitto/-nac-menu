...
function dateOverlapScore(row = {}, vaultPeriod = null) {
...
function recencyScore(row = {}, options = {}) {
...
function metadataRelevanceScore(row = {}, queryContext = {}, options = {}) {
  let score = 0;
  ...
  const requestedReportTypes =
    options.reportTypes?.length ? options.reportTypes : inferReportTypesFromQuery(queryContext.normalized);
  if (requestedReportTypes.length && requestedReportTypes.includes(row.report_type)) score += 16;

  score += dateOverlapScore(row, options.vaultPeriod);
 ...
 */
export function scoreChunkRelevance(row = {}, queryContext = {}, options = {}) {
...
export function rankDocumentSearchChunks(rows = [], searchTerms = "", options = {}) {
