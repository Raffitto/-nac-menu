...
import { fetchInventoryReferenceData, fetchInvoiceHistory, fetchIngredients, fetchInventoryStaffAccess } from "../lib/inventoryApi";
...
describe("InventoryApp", () => {
  ...
  test("shows invoice review and ingredients tabs", async () => {
    render(<InventoryApp />);
  ...
  test("switches to ingredients without breaking invoice review access", async () => {
    render(<InventoryApp />);
    ...
    await waitFor(() => {
      expect(fetchInvoiceHistory).toHaveBeenCalled();
    });
  });
});
