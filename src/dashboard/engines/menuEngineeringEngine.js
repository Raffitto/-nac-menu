...
export function classifyMenuItems(funnels = []) {
  ...
  const withOrders = funnels.filter((f) => f.orders > 0 || f.item_opens > 0);
  ...
  const maxConv = Math.max(...withOrders.map((f) => clampMetric(f.conversion_pct, 0, 100)), 1);
  ...
  return withOrders.map((f) => {
    ...
    const convScore = ((f.conversion_pct || 0) / maxConv) * 100;
