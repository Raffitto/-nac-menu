...
function resolveNacFilenameYear(monthIndex, explicitYear, referenceDate = /* @__PURE__ */ new Date()) {
...
function parseNacDateFromFilename(text = "", referenceDate = /* @__PURE__ */ new Date()) {
...
function parseDailyLogbookText(text, context, intermediate = null) {
  ...
  const periodStart = parseIsoDate(extracted.log_date) || parseNacDateFromText(text) || parseNacDateFromFilename(logbookFilenameHint(context)) || context.periodStart || null;
  ...
  const confidenceMeta = explainConfidence(rawConfidence, {
    coreMatched: receptionMatched + matchedMeta + (filenameLogbook && branchId ? 1 : 0) + (periodStart ? 1 : 0),
    coreRequired: 2,
