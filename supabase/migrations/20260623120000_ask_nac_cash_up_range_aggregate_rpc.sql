-- Ask NAC P0: server-side cash-up range aggregation (single round-trip for YTD / wide ranges).
-- Replaces sequential monthly raw-fact fetches when daily breakdown is not required.
...
comment on function public.get_vault_cash_up_range_aggregate(date, date, text, boolean) is
  'Aggregates cash-up structured facts for a date range in one query. Used by Ask NAC for YTD and wide-range performance.';
