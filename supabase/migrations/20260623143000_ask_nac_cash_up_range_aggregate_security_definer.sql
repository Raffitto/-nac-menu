-- P0 hotfix: RPC timed out under RLS (security invoker). Use security definer with explicit scope checks + index.

create index if not exists idx_ask_nac_facts_cash_up_range
  on public.ask_nac_structured_facts (branch_id, period_end, metric_key)
  where report_type = 'cash_up' and archived_at is null;
...
begin
  ...
  daily_metrics as (
    select
      business_date,
      max(case when metric_key = 'net_sales' and not (dimensions ? 'platform') then metric_value end) as net_sales_agg,
      max(case when metric_key = 'total_sales' and not (dimensions ? 'platform') then metric_value end) as total_sales_agg,
      max(case when metric_key = 'gross_sales' and not (dimensions ? 'platform') then metric_value end) as gross_sales_agg,
      max(case when metric_key = 'guest_count' and not (dimensions ? 'platform') then metric_value end) as guest_count,
      max(case when metric_key = 'order_count' and not (dimensions ? 'platform') then metric_value end) as order_count,
      max(case when metric_key = 'delivery_sales' and not (dimensions ? 'platform') then metric_value end) as delivery_sales_agg,
      coalesce(
        sum(case when metric_key = 'delivery_sales' and dimensions ? 'platform' then metric_value else 0 end),
        0
      ) as delivery_sales_platform,
      max(case when metric_key = 'delivery_orders' and not (dimensions ? 'platform') then metric_value end) as delivery_orders_agg,
      coalesce(
        sum(case when metric_key = 'delivery_orders' and dimensions ? 'platform' then metric_value else 0 end),
        0
      ) as delivery_orders_platform
    from filtered
  ...
  platform_raw as (
    select
      case
        when lower(coalesce(dimensions->>'platform', '')) like '%jahez%' then 'jahez'
        when lower(coalesce(dimensions->>'platform', '')) like '%chefz%'
          or lower(coalesce(dimensions->>'platform', '')) like '%the chefz%' then 'chefz'
        when lower(coalesce(dimensions->>'platform', '')) like '%keeta%' then 'keeta'
        when lower(coalesce(dimensions->>'platform', '')) like '%hunger%' then 'hunger'
        else null
      end as platform,
  ...
  platform_totals as (
    select
      coalesce(sum(sales), 0) as total_platform_sales,
      coalesce(sum(orders), 0) as total_platform_orders
    from platform_raw
  ),
...
comment on function public.get_vault_cash_up_range_aggregate(date, date, text, boolean) is
  'Aggregates cash-up structured facts for a date range. Security definer with explicit vault scope checks (P0 perf).';
