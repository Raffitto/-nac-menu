-- One canonical Cash Up fact per branch, day, and metric.
-- A living workbook revision wins over an older PDF snapshot of the same day.
-- Identical revisions are not summed.
...
begin
  ...
  with canonical as (
    select distinct on (
      f.period_end,
      f.metric_key,
      coalesce(f.dimensions->>'platform', '')
    )
    ...
    order by
      ...
      f.metric_key,
      coalesce(f.dimensions->>'platform', ''),
      (f.file_version_id is not null) desc,
      (coalesce(f.source_row_ref, '') like 'pdf-day%') asc,
      f.created_at desc
  ),
  daily_metrics as (
    select
