...
-- Patches get_bi_dashboard to use nac_is_category_nav_event for category funnel + top_categories.
  ...
  select jsonb_build_object(
    ...
    'top_categories', coalesce(
      (
        ...
        from (
          select
            category_id as id,
            count(*) filter (where public.nac_is_category_nav_event(event_type))::bigint as opens,
          ...
          order by greatest(
            ...
            count(*) filter (where public.nac_is_category_nav_event(event_type))
          ) desc
          limit 10
        ) v
      ),
    ...
    'bounce_sessions', (
      ...
      from (
        ...
        having count(*) <= 2
          and count(*) filter (
            where event_type in ('item_open','category_open','menu_tab_open','section_open','search_used','search_submit','add_on_click','item_impression')
          ) = 0
      ) b
    ),
    'deep_sessions', (
    ...
    'funnel', jsonb_build_object(
      ...
      'category_opens', (select count(distinct session_id)::bigint from filtered where public.nac_is_category_nav_event(event_type)),
          ...
          select
            ...
            round(
              (count(*) filter (where event_type = 'item_open')::numeric
               / nullif(count(*) filter (where public.nac_is_category_nav_event(event_type)), 0)) * 100,
              1
            ) as engagement_ratio
          from filtered
          where category_id is not null
          group by category_id
          having count(*) filter (where public.nac_is_category_nav_event(event_type)) >= 3
      ...
      (
        ...
        from (
          select
            ...
            count(*) filter (where public.nac_is_category_nav_event(event_type))::bigint as co
          from filtered
          where category_id is not null
          group by category_id
          having count(*) filter (where public.nac_is_category_nav_event(event_type)) >= 5
          order by (count(*) filter (where event_type = 'item_open')::numeric
                    / count(*) filter (where public.nac_is_category_nav_event(event_type))) desc
          limit 1
        ) tc
      ),
      '{}'::jsonb
