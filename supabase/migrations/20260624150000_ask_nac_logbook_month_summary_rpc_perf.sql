-- Perf: avoid per-row ask_nac_vault_can_read_scope in logbook month RPC.
-- Branch is validated once at entry; rows are branch-scoped (branch_id = p_branch_id).
...
begin
  ...
  where f.report_type = 'daily_logbook'
    ...
    and public.ask_nac_vault_can_read_sensitivity(f.sensitivity_level)
    and public.ask_nac_vault_department_allowed(f.department);
